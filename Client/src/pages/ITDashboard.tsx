...
const ITDashboard = () => {
  ...
  useEffect(() => {
    axios.get('http://localhost:5293/ITDashboard/monthly-ticket-counts')
      .then(response => {
      ...
      axios.get('http://localhost:5293/ITDashboard/available-agent-count')
      .then(response => {
      ...
      .then((response) =>{ setAverageResponseTime(response.data);})
      .catch((error) => {console.log(error)});
      
      
      axios.get('http://localhost:5293/ITDashboard')
  ...
  const chartData = {
    data: {
      ...
      datasets: [
        {
          label: 'Good',
          data: ticketSatisfaction.map((satisfaction) => satisfaction.goodCount),
          backgroundColor: 'green'
        },
        {
          label: 'Better',
          data: ticketSatisfaction.map((satisfaction) => satisfaction.betterCount),
          backgroundColor: 'yellow'
        },
        {
          label: 'Worst',
          data: ticketSatisfaction.map((satisfaction) => satisfaction.worstCount),
          backgroundColor: 'red'
        }
      ]
    }
    ...
    <div>
      <Container>
        ...
        <div className="row mb-6" style={{ margin: "0px 0 0 65px" }}>
                  ...
                  <CardDashboard  name="Average Response Time" count={averageResponseTime}  />
        </div>
        ...
        {/* code for TicketBarChart component */}
	      <h4 style={{margin:"0px 0 0 65px"}}>Ticket Satisfaction</h4>
            <div style={{margin:"0px 0 0 65px"}}>
              <TicketBarChart data={{labels: ticketSatisfaction.map((satisfaction) => satisfaction.month),
      datasets: [
      {
        label: 'Good',
        data: ticketSatisfaction.map((satisfaction) => satisfaction.goodCount),

...
const EmployeeRequestTable: React.FC = () => {
  ...
  const fetchEmployeeRequests = async () => {
    try {
      const response = await axios.get<EmployeeRequest[]>('http://localhost:5087/api/EmployeeRequest');
      setEmployeeRequests(response.data);
      setAcceptedRequests(response.data.filter((request) => request.isAccepted));
    } catch (error) {
      console.log(error);
    }
  };
  ...
  const handleToggleAcceptance = async (id: number) => {
    try {
      const updatedRequests = employeeRequests.map((request) => {
        if (request.id === id) {
          const updatedRequest = {
            ...
            acceptedDateTime: request.isAccepted ? '' : new Date().toISOString(),
          };
      ...
      await axios.put(`http://localhost:5087/api/EmployeeRequest/${id}`, { isAccepted: updatedRequests.find((r) => r.id === id)?.isAccepted });
    } catch (error) {
      console.log(error);
    }
  };
        ...
        <tbody>
          {employeeRequests.map((request) => (
            <tr key={request.id}>
              ...
              <td>
                <button
                  className={`btn ${request.isAccepted ? 'btn-success' : 'btn-danger'}`}

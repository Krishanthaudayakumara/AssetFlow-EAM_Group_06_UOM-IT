...
namespace Server.Controllers
{
    ...
    public class ITDashboardController : ControllerBase
    {
       ...
       [HttpGet("monthly-ticket-counts")]
        ...
        {
            var availableAgentCount = _context.Agents.Count(a => a.AgentStatus == "Available");

            var availableAgentCountDTO = new AvailableAgentCountDTO
            {
        ...
        {
            var solvedTicketCount = _context.Tickets.Count(t => t.TicketStatus == "Solved");

            var solvedTicketCountDTO = new SolvedTicketCountDTO
            {
      ...
      [HttpGet("avg-response-time")]
        ...
        [HttpGet]
        public ActionResult<IEnumerable<TicketSatisfactionDTO>> GetTicketSatisfactionByMonth()
        {
            var feedbacks = _context.Feedbacks.Include(f => f.Ticket);

            var satisfactionByMonth = feedbacks.GroupBy(f => new { f.CreatedDate.Year, f.CreatedDate.Month })
                .Select(group => new TicketSatisfactionDTO
                {
